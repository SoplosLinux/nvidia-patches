#!/bin/bash
# nvidia-610-deb14-pahole-fix.sh
#
# Fix: NVIDIA DKMS 610.x fails to build on Debian testing kernels with CONFIG_DEBUG_INFO_BTF_MODULES=y
#
# Root cause:
...
#
# Affected:  nvidia-dkms 610.x, Debian testing kernels >= 7.0.4 with BTF_MODULES=y
# Not affected: custom kernels without CONFIG_DEBUG_INFO_BTF_MODULES=y
#
# Usage: sudo bash nvidia-610-deb14-pahole-fix.sh [kernel-version]
...
# Find nvidia 610.x version in dkms
DKMS_ENTRY=$(dkms status 2>/dev/null | grep -E '^nvidia/' | grep '/610\.' | head -1)
if [ -z "$DKMS_ENTRY" ]; then
    echo "WARNING: Could not detect nvidia 610.x in dkms, skipping rebuild." >&2
