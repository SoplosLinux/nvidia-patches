...
#
# Affected:  nvidia-kernel-dkms 590.x, Debian kernel >= 7.0.13+deb14-amd64
# Not affected: custom kernels without CONFIG_DEBUG_INFO_BTF_MODULES=y
#
# Usage: sudo bash nvidia-590-deb14-btf-fix.sh [nvidia-version]
#   nvidia-version defaults to the newest installed nvidia 590.x DKMS version
...
if [ -z "$NVIDIA_VER" ]; then
    NVIDIA_VER=$(dkms status 2>/dev/null | grep -E '^nvidia/' | grep '/590\.' | grep -oP '^nvidia/\K[0-9.]+' | head -1)
    if [ -z "$NVIDIA_VER" ]; then
        echo "ERROR: Could not detect nvidia 590.x in dkms." >&2
        echo "Pass the version manually: sudo bash $0 590.48.01" >&2
        exit 1
    fi
